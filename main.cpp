...
void CrowApp::test(int port){
    try {
        ...
        // Set up Asio for HTTP request
        asio::io_context io_context;
        asio::ip::tcp::socket socket(io_context);
        asio::ip::tcp::resolver resolver(io_context);
        asio::connect(socket, resolver.resolve("localhost", to_string(port)));

        std::string request = "POST /resize_image HTTP/1.1\r\n"
                        "Host: localhost\r\n"
                        "Connection: close\r\n"
        ...
        std::string response;
        asio::read(socket, asio::dynamic_buffer(response));
        std::cout << "Response:\n" << response << std::endl;
